...
import { EditorView } from "../../../lib/tprose/react";
...
import { Editor, selectEnd, setContent } from "../../../lib/tprose/editor";
import { DOMParser, JSONNode } from "../../../lib/tprose/model";
import { StarterPlugin } from "../../../lib/tprose/plugins/starter";
...
const MessageInput: FC<OwnProps & StateProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const parseHtml = (): JSONNode => {
      if (editorRef.current) {
        const document = new global.DOMParser().parseFromString(
          html,
          "text/html"
        );
        const content = new DOMParser(editorRef.current.schema).parseFragment(
          document.body
        );
        return { type: "doc", content: content.toJSON() };
      }
      return { type: "doc" };
    };

    if (html !== inputRef.current!.innerHTML) {
      editorRef.current?.apply(setContent(parseHtml()), selectEnd());
    }
      ...
      >
        <div className={inputScrollerContentClass}>
          ...
          <EditorView
            ...
            dir="auto"
            tabIndex={0}
            plugins={[new StarterPlugin()]}
            onKeyDown={handleKeyDown}
            onUpdate={(editor) => onUpdate(editor.dom.innerHTML)}
            onFocus={!isNeedPremium ? onFocus : undefined}
            onBlur={!isNeedPremium ? onBlur : undefined}
          />
          {!forcedPlaceholder && (
